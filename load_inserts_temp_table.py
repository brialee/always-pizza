...
def createTempTable(conn):
	with conn.cursor() as cursor:
		...
		print(f"Created Unlogged {TempTableName}")

def loadTemp(conn, icmdlist):
...
def appendTempToIndexed(conn):
	...
	conn.commit()
	elapsed = time.perf_counter() - start
	print(f'Finished Appending Temp to Indexed Table. Elapsed Time: {elapsed:0.4} seconds')
...
def main():
	...
	loadTemp(conn, cmdlist)
	appendTempToIndexed(conn)
