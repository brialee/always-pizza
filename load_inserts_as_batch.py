...
def loadTableBatch(conn, censusDict, TableName=TableName):
	# Construct insert statement
	statement = "insert into CensusData values("
	valString = '%s,' * len(censusDict[0].keys())
	statement += valString.rstrip(',') + ");"
	...
	# Construct list of tuples with values to be inserted
	valList = []
	for item in censusDict:
		valList.append(tuple(item.values()))

	cur = conn.cursor()
	start = time.perf_counter()
	psycopg2.extras.execute_batch(cur, statement, valList)
	conn.commit()
	elapsed = time.perf_counter() - start
	print(f'Finished Loading into Table as batch. Elapsed Time: {elapsed:0.4} seconds')
