...
def createUnloggedTable(conn):
	with conn.cursor() as cursor:
		...
		print(f"Created Unlogged {StagingTableName}")

def loadStaging(conn, icmdlist):
	...
	with conn.cursor() as cursor:
		...
		print(f'Finished Loading into Staging Table. Elapsed Time: {elapsed:0.4} seconds')

def appendStagingToIndexed(conn):
	...
	conn.commit()
	elapsed = time.perf_counter() - start
	print(f'Finished Appending Staging to Indexed Table. Elapsed Time: {elapsed:0.4} seconds')
...
def main():
	...
	loadStaging(conn, cmdlist)
	appendStagingToIndexed(conn)
